...
export default function ReadingProgress({ isOpen, onClose, onNavigate }: ReadingProgressProps) {
  const [completedChapters, setCompletedChapters] = useState<Record<string, boolean>>(() => {
    const saved = localStorage.getItem('gbp_completed_chapters');
    return saved ? JSON.parse(saved) : {};
  });

  const [readingLogs, setReadingLogs] = useState<Record<string, number>>(() => {
    const saved = localStorage.getItem('gbp_reading_logs');
    return saved ? JSON.parse(saved) : {};
  });

  const [lastRead, setLastRead] = useState(() => {
    const last = localStorage.getItem('gbp_last_ref');
    return last ? JSON.parse(last) : { b: 'GEN', c: 1 };
  });
      ...
      <div className="relative w-full max-w-lg h-full md:h-[95vh] bg-white md:rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-500">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          ...
          <div className="bg-indigo-600 text-white px-8 py-3 flex items-center justify-between gap-4">
            ...
            <p className="text-[10px] bg-black/20 px-2 py-1 rounded-lg">일일 목표 3.9장</p>
          </div>
                    ...
                    return (
                      <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
                        <div className="relative w-full group/bar">
                           {day.count > 0 && (
                             <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-black p-1 bg-white text-black rounded shadow-lg opacity-0 group-hover/bar:opacity-100 transition-opacity">
                               {day.count}
                             </span>
                           )}
                           <div 
               ...
               <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/5 relative z-10">
                  <div className="flex gap-4">
                     ...
                     <div className="flex flex-col">
                        ...
                        <span className="text-sm font-black">{totalChapters - completedCount}장</span>
                     </div>
                  </div>
          ...
          <div className="px-6 py-2 grid grid-cols-2 gap-4">
             <button 
                ...
                className="bg-slate-900 text-white rounded-[2rem] p-6 flex flex-col items-start gap-4 shadow-xl hover:bg-black transition-all group overflow-hidden relative"
              >
             ...
             <button className="bg-white border border-slate-100 rounded-[2rem] p-6 flex flex-col items-start gap-4 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all group overflow-hidden relative">
          ...
          <div className="px-6 py-4">
             <p className="text-center text-xs text-gray-500">앞으로 매일 <span className="text-blue-500 font-bold">3~4개 장</span>씩 읽으시면 됩니다. (3.9장)</p>
          </div>

          {/* Books List Section (Clean & Modern Check-style) */}
          ...
          {/* Floating Progress Bar at bottom - Modern Slate / Glass */}
          <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-lg bg-slate-900/95 backdrop-blur-xl text-white p-6 pb-10 space-y-4 shadow-[0_-20px_50px_rgba(0,0,0,0.3)] animate-in slide-in-from-bottom duration-700">
             <div className="flex justify-between items-center px-2">
                <div className="flex flex-col">
                   ...
                   <span className="text-[11px] font-bold">2026.1.2 ~ 2026.12.31</span>
                </div>
                <div className="text-right">
                   <div className="text-[10px] font-black opacity-40 uppercase tracking-widest mb-1">Total Completion</div>
                   <div className="text-xl font-black text-indigo-400">31.3%</div>
                </div>
             </div>
             ...
             <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                   ...
                   <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-slate-600 transition-all duration-1000" style={{ width: `22.3%` }} />
                   </div>
                </div>
             </div>
          </div>
